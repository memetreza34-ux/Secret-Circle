...
### Direkte Hub-Bedienung

- das frühere mehrdeutige **„Spiel verlassen“** wurde entfernt
- **„Beenden & speichern“** schreibt nur den bewusst abgeschlossenen Sessionstand
- **„Abbrechen & verwerfen“** verlangt eine Bestätigung und schreibt weder Verlauf noch Statistik
- Escape verwendet denselben bestätigten Abbruchpfad statt still zu speichern
- **„Runde überspringen“** ist als gemeinsame Hub-Aktion vorhanden und vergibt keinen Punkt
- bereits gestartete zeitgesteuerte Runden werden beim bewussten Speichern als Runde berücksichtigt, damit Treffer nicht verloren gehen
...
1. **Automatisierte Browserausführung:** die vorbereiteten Playwright-Verträge benötigen einen funktionierenden Runner.
2. **Punkte/Sieger:** die Regeln der 15 Kernspiele müssen vollständig als Produktvertrag dokumentiert und anschließend real geprüft werden.
3. **Dependency-Lock:** `package-lock.json` und `npm ci` bleiben offen.
4. **Geräte und reale Gruppen:** Android, iPhone, Tablet, PWA-Update, Sperrbildschirm und echte Partyabende fehlen noch.
5. **Inhalte und Recht:** redaktionelle Alters-, Fan-Content- und Rechtsprüfung ist noch nicht abgeschlossen.
...
2. Kerninhalte und Altersstufen redaktionell prüfen
3. Android-, iPhone-, Tablet-, PWA-Update-, Sperrbildschirm- und reale Gruppentests
4. Dependency-Lock erzeugen und CI auf `npm ci` umstellen
5. Branch Protection aktivieren, sobald Actions zuverlässig läuft
