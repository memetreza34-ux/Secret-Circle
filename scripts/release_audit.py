...
checks = {
    ...
    'cache_v28': "const CACHE='secret-circle-v28'" in sw,
    ...
    'viral_catalog_v5': 'version: 5' in viral and 'viralGameIds' in viral and 'allFastGameIds' in viral,
    'routing_v6': 'version: 6' in routing and "require('./party-viral-catalog.js')" in routing,
    ...
    'viral_engine_family': all(marker in viral_runtime for marker in (
        'renderFingerDown', 'renderGuessPrice', 'renderHigherLower', 'renderKnowMeBest',
        'renderHearMeOut', 'renderHotSeat', 'renderStoryChain', 'renderFinishSentence'
    )),
    'single_engine_loader': all(marker in loader for marker in ('party-viral-modes.js', 'party-mega-modes.js', 'party-quick-modes.js')),
    ...
    'viral_syntax_gates': all(marker in package.get('scripts', {}).get('check', '') for marker in ('party-viral-catalog.js', 'party-viral-modes.js', 'quick-loader.js')),
    'viral_unit_gate': 'tests/party-viral-catalog.test.js' in package.get('scripts', {}).get('test', ''),
...
if len(unit_tests) < 12 or len(e2e_suites) < 26 or not cross_suites:
    raise SystemExit('Release test matrix is incomplete.')
for required in ('party-trending-catalog.test.js', 'party-mega-catalog.test.js', 'party-viral-catalog.test.js'):
    if required not in unit_tests:
        raise SystemExit(f'Critical unit test missing: {required}')
for required in ('party-quick-modes.spec.js', 'party-mega-modes.spec.js', 'party-viral-modes.spec.js', 'offline.spec.js'):
...
for forbidden in ['eval(', 'new Function(', 'document.write(', 'http://']:
    for relative in [
        ...
        'quick-loader.js', 'party-hub.js', 'party-advanced-runner.js'
    ]:
...
print(json.dumps({
    ...
    'pwa_cache': 'secret-circle-v28',
    'visible_games': base_games + expansion_games + trending_games + mega_games + viral_games,
    'playable_games': 45,
    ...
    'price_cards_are_game_values': True,
    'offline_first': True,
