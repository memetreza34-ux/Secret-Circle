...
for relative in required:
    if not (ROOT / relative).is_file(): violations.append(f'Missing Quick replacement contract file: {relative}')
...
for marker in (
    'version: VERSION', "created: 'secret-circle-party-created-active-v1'",
    "viral: 'secret-circle-party-viral-active-v1'", "mega: 'secret-circle-party-mega-active-v1'",
    "quick: 'secret-circle-party-quick-active-v1'", 'function plausibleSnapshot(value)',
    'function authorizeStart(root, catalog, gameId)', 'event.stopImmediatePropagation();',
    'blockPagehideRetry = true;', 'root.location?.reload?.()'
):
    if marker not in guard: violations.append(f'Quick replacement guard marker missing: {marker}')
...
for marker in (
    "families: ['quick', 'mega', 'viral', 'created']", 'sameGameReplacementConfirmed: true',
    'crossGameFamilyReplacementConfirmed: true', 'crossGameSnapshotPreserved: true'
):
    if marker not in unit: violations.append(f'Quick replacement unit marker missing: {marker}')
...
for marker in (
    ...
    if marker not in e2e: violations.append(f'Quick replacement browser marker missing: {marker}')

for marker in (
    'every fast engine loads controls and replacement guard before its engine',
    "sources.indexOf('party-session-controls.js')", "sources.indexOf('quick-session-replacement-guard.js')",
    'expect(guardIndex).toBeGreaterThan(controlsIndex)', 'expect(engineIndex).toBeGreaterThan(guardIndex)'
):
    if marker not in controls_e2e: violations.append(f'Quick replacement load-order browser marker missing: {marker}')
...
for relative in ('quick-session-replacement-guard.js','tests/quick-session-replacement-guard.test.js','tests/e2e/quick-session-replacement.spec.js','tests/e2e/party-session-controls.spec.js'):
    if f'node --check {relative}' not in scripts.get('check', ''): violations.append(f'Quick replacement file missing from syntax preflight: {relative}')
...
if './quick-session-replacement-guard.js' not in sw: violations.append('Quick replacement guard missing from offline core.')
cache = re.search(r"const CACHE='secret-circle-v(\d+)'", sw)
if not cache or int(cache.group(1)) < 56: violations.append('Quick replacement hardening requires offline cache generation v56 or newer.')
