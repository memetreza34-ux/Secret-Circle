...
contracts = {
    ...
    'quick-loader.js': [
        "REPLACEMENT_GUARD_SOURCE = 'quick-session-replacement-guard.js'",
        "WAVE_ONE_SOURCE = 'party-wave-one-modes.js'",
        "WAVE_ONE_IMPOSTER_SOURCE = 'party-wave-one-imposter-modes.js'",
        'catalog.waveOneImposterGameIds?.includes(gameId)', 'catalog.waveOneQuizGameIds?.includes(gameId)',
        'replacementGuardReady = false', 'plan.push(REPLACEMENT_GUARD_SOURCE)', 'version: 9'
    ],
    ...
    'party-wave-one-imposter-modes.js': [
        "ACTIVE_KEY = 'secret-circle-party-quick-active-v1'", 'waveOneImposterGameIds',
...
for module in (
    'party-expansion.js', 'party-mega-catalog.js', 'party-core-release-catalog.js', 'party-core-classic-content.js',
    'party-wave-one-catalog.js', 'party-wave-one-imposter-catalog.js', 'party-wave-one-modes.js',
    'party-wave-one-imposter-modes.js', 'party-session-controls.js', 'party-hub-timers.js',
    'party-hub-resume-guard.js', 'party-hub-round-state.js', 'party-hub-a11y.js',
    'secondary-surface-a11y.js', 'word-imposter-resume-guard.js', 'advanced-resume-guard.js',
    'party-advanced-runner.js', 'advanced-privacy-guard.js', 'quick-session-replacement-guard.js', 'quick-loader.js'
):
    if f'node --check {module}' not in syntax_gate:
        violations.append(f'Production module missing from syntax gate: {module}')
for test in (
    'tests/party-mega-catalog.test.js', 'tests/party-wave-one-catalog.test.js',
    'tests/party-wave-one-imposter-catalog.test.js', 'tests/core-content-quality.test.js',
    'tests/hub-resume-contract.test.js', 'tests/hub-control-contract.test.js', 'tests/party-session-controls.test.js',
    'tests/advanced-resume-guard.test.js', 'tests/advanced-resume-contract.test.js', 'tests/quick-loader.test.js',
    'tests/quick-session-replacement-guard.test.js', 'tests/accessibility-contract.test.js', 'tests/manifest-icons.test.js'
):
    if test not in unit_gate:
        violations.append(f'Critical architecture test missing from npm test: {test}')
for e2e in (
    'tests/e2e/quick-timer-resume.spec.js', 'tests/e2e/quick-background-pause.spec.js',
    'tests/e2e/wave-one-quiz.spec.js', 'tests/e2e/wave-one-imposter.spec.js'
):
    if f'node --check {e2e}' not in syntax_gate:
        violations.append(f'Critical E2E missing from syntax gate: {e2e}')
for audit in (
    'scripts/wave_one_quiz_audit.py', 'scripts/wave_one_imposter_audit.py', 'scripts/advanced_integrity_audit.py',
    'scripts/quick_session_replacement_audit.py', 'scripts/quick_timer_resume_audit.py',
    'scripts/quick_bfcache_resume_audit.py', 'scripts/quick_background_pause_audit.py',
    'scripts/quick_hidden_snapshot_audit.py', 'scripts/backup_contract_audit.py', 'scripts/core_content_audit.py',
    'scripts/reference_content_audit.py', 'scripts/asset_provenance_audit.py', 'scripts/media_inventory_audit.py',
    'scripts/public_release_placeholder_audit.py', 'scripts/hub_a11y_contract_audit.py',
    'scripts/secondary_surface_a11y_contract_audit.py', 'scripts/operator_release_contract_audit.py',
...
for asset in (
    './backup-schema-registry.js', './word-imposter-resume-guard.js', './party-expansion.js',
    './party-mega-catalog.js', './party-core-release-catalog.js', './party-core-classic-content.js',
    './party-wave-one-catalog.js', './party-wave-one-imposter-catalog.js', './party-wave-one-modes.js',
    './party-wave-one-imposter-modes.js', './party-hub-timers.js', './party-hub-resume-guard.js',
    ...
    './quick-session-replacement-guard.js', './quick-loader.js', './session-ledger.js',
    './party-session-controls.js', './icon.svg', './icon-192.png', './icon-512.png'
):
    if asset not in sw:
...
print(json.dumps({
    ...
    'quick_replacement_guard_version': 2,
    'quick_loader_version': 9,
    'quick_session_replacement_audit_required': True,
    'session_controls_version': 5,
    'quick_timer_resume_audit_required': True,
    'quick_bfcache_resume_audit_required': True,
    'quick_background_pause_audit_required': True,
    'quick_hidden_snapshot_audit_required': True,
    'wave_one_quiz_audit_required': True,
    'wave_one_imposter_audit_required': True,
    'wave_one_playable_labs': ['party-quiz', 'fact-or-fake', 'undercover-similar-word', 'no-word-imposter'],
    'wave_one_shared_quiz_runner': True,
    'wave_one_shared_imposter_runner': True,
    'wave_one_private_handoff_concealment': True,
    'wave_one_reference_safe_text_content': True,
