...
production_js = [
    'runtime-guard.js','setup-ux.js','privacy-guard.js','wake-lock.js','game-engine.js','role-assignment.js','word-packs.js','data-store.js',
    'word-imposter-resume-guard.js','app.js','backup-schema-registry.js','session-ledger.js','party-session-controls.js','party-catalog.js',
    'party-expansion.js','party-trending-catalog.js','party-mega-catalog.js','party-viral-catalog.js','party-core-release-catalog.js','party-core-classic-content.js',
    'party-routing.js','party-wave-one-catalog.js','party-wave-one-imposter-catalog.js','party-wave-one-writing-catalog.js','party-wave-one-voting-catalog.js',
    'party-wave-one-bluff-catalog.js','party-wave-one-clue-catalog.js','party-release-structure.js','party-filter-state.js','party-search-assist.js',
    'game-creator.js','creator-page.js','party-custom-packs.js','party-hub-timers.js','party-hub-resume-guard.js','party-hub-round-state.js',
    'party-hub.js','party-hub-plus.js','party-hub-polish.js','party-hub-a11y.js','secondary-surface-a11y.js','party-guide.js','party-night.js','party-data-tools.js',
    'party-advanced.js','advanced-resume-guard.js','party-advanced-runner.js','advanced-privacy-guard.js','party-advanced-preferences.js',
    'party-quick-modes.js','party-mega-modes.js','party-viral-modes.js','party-created-modes.js','party-wave-one-modes.js','party-wave-one-imposter-modes.js',
    'party-wave-one-writing-modes.js','party-wave-one-voting-modes.js','party-wave-one-bluff-modes.js','party-wave-one-clue-modes.js',
...
required_contract_files = [
    'manifest.webmanifest','icon.svg','icon-192.png','icon-512.png','assets/manifests/asset-provenance.json','tests/manifest-icons.test.js',
    'tests/accessibility-contract.test.js','scripts/asset_provenance_audit.py','scripts/media_inventory_audit.py','scripts/hub_a11y_contract_audit.py',
    'scripts/secondary_surface_a11y_contract_audit.py','scripts/advanced_integrity_audit.py','scripts/wave_one_quiz_audit.py',
    'scripts/wave_one_imposter_audit.py','scripts/wave_one_writing_audit.py','scripts/wave_one_remaining_audit.py','scripts/quick_session_replacement_audit.py',
    'scripts/quick_timer_resume_audit.py','scripts/quick_bfcache_resume_audit.py','scripts/quick_background_pause_audit.py','scripts/quick_hidden_snapshot_audit.py',
    'scripts/backup_contract_audit.py','tests/e2e/quick-background-pause.spec.js','tests/e2e/wave-one-quiz.spec.js','tests/e2e/wave-one-imposter.spec.js',
    'tests/e2e/wave-one-writing.spec.js','tests/e2e/wave-one-remaining.spec.js'
]
...
for marker in (
    'Stabile Identitäten','Versionierte Daten und Backups','Katalog- und Contentarchitektur','Hub- und Timergrenzen','Lokale Transaktionen und Exact-once',
    'Datenschutz und Security durch Architektur','Offline- und Updatevertrag','Accessibility als Definition of Done','Inhalts- und Rechtevertrag','Testpyramide',
    'Performance und Assets','Betrieb, Deprecation und Rollback','Quick-/Mega-/Viral-/Creator-Session-Ersatz – v56','Quick-Family Timer Resume – v57',
    'Quick-Family BFCache Resume – v58','Quick-Family Background Pause – v59','Quick-Family Hidden Snapshot – v60','Expansion Wave 1 – v61',
    'Expansion Wave 1 Imposter – v62','Expansion Wave 1 Writing – v63','Expansion Wave 1 Complete – v64'
):
...
for relative in production_js:
    path=ROOT/relative; source=read(relative); lines=len(source.splitlines())
    if lines > 1000: violations.append(f'{relative} has {lines} lines; split it before 1000 lines.')
    if path.stat().st_size > 100000: violations.append(f'{relative} exceeds the 100 KB module limit.')
...
def check_order(source,names,context):
    ...
    for name in names:
        ...
        if position < 0: violations.append(f'{context} missing ordered module: {name}'); return
        positions.append(position)
    if positions != sorted(positions): violations.append(f'{context} module order is invalid: {" -> ".join(names)}')

check_order(party_page,catalog_chain,'party.html'); check_order(quick_play,catalog_chain,'quick-play.html')
...
contracts={
 ...
 'quick-loader.js':["WAVE_ONE_SOURCE = 'party-wave-one-modes.js'","WAVE_ONE_IMPOSTER_SOURCE = 'party-wave-one-imposter-modes.js'","WAVE_ONE_WRITING_SOURCE = 'party-wave-one-writing-modes.js'","WAVE_ONE_VOTING_SOURCE = 'party-wave-one-voting-modes.js'","WAVE_ONE_BLUFF_SOURCE = 'party-wave-one-bluff-modes.js'","WAVE_ONE_CLUE_SOURCE = 'party-wave-one-clue-modes.js'",'catalog.waveOneClueGameIds?.includes(gameId)','catalog.waveOneBluffGameIds?.includes(gameId)','catalog.waveOneVotingGameIds?.includes(gameId)','catalog.waveOneWritingGameIds?.includes(gameId)','catalog.waveOneImposterGameIds?.includes(gameId)','catalog.waveOneQuizGameIds?.includes(gameId)','version: 11'],
 'party-wave-one-catalog.js':["id: 'party-quiz'","id: 'fact-or-fake'","id: 'undercover-similar-word'","id: 'no-word-imposter'",'waveOneQuizGameIds','waveOneImposterGameIds','version: 3'],
 'party-wave-one-imposter-catalog.js':['Konsolidierter Wave-1-Katalog fehlt.','waveOneImposterGameIds','return base;'],
 'party-wave-one-writing-catalog.js':["id: 'fill-blank-battle'","id: 'who-wrote-it'",'waveOneWritingGameIds','version: 4'],
 'party-wave-one-voting-catalog.js':["id: 'percent-guess'","id: 'party-bracket'",'waveOneVotingGameIds','version: 5'],
 'party-wave-one-bluff-catalog.js':["id: 'bluff-trivia'",'waveOneBluffGameIds','version: 6'],
 'party-wave-one-clue-catalog.js':["id: 'password-one-word'",'waveOneClueGameIds','version: 7'],
 'party-wave-one-modes.js':["L.completionId('wave1', game.id, active.sessionId)","gameId === 'party-quiz'","gameId === 'fact-or-fake'"],
 'party-wave-one-imposter-modes.js':["L.completionId('wave1-imposter', game.id, active.sessionId)",'function concealPrivate()','function resolveVotes()','function submitGuess()'],
 'party-wave-one-writing-modes.js':["L.completionId('wave1-writing', game.id, active.sessionId)",'function concealEntry()','function normalizeAnswers(','function chooseWinner(','function submitAuthorGuess('],
 'party-wave-one-voting-modes.js':["L.completionId('wave1-voting', game.id, active.sessionId)",'function pointsForEstimate(','function deriveBracket(','cardIndex'],
 'party-wave-one-bluff-modes.js':["L.completionId('wave1-bluff', game.id, active.sessionId)",'function concealPrivate()','function finishVotingScore()','candidate.author === voter'],
 'party-wave-one-clue-modes.js':["L.completionId('wave1-clue', game.id, active.sessionId)",'function concealSecret()',"if (/\\s/.test(clue))",'cardIndex'],
 ...
 'scripts/wave_one_imposter_audit.py':['wave_one_imposter_audit','shared_imposter_runner','private_handoff_concealment'],
 'scripts/wave_one_writing_audit.py':['wave_one_writing_audit','shared_writing_runner','private_entry_concealment'],
 'scripts/wave_one_remaining_audit.py':['wave_one_remaining_audit','wave_one_complete','adult_content'],
 'scripts/advanced_integrity_audit.py':['advanced_integrity_audit','resume_guard_version'],
 'scripts/quick_session_replacement_audit.py':['quick_session_replacement_audit','families_guarded'],
 'scripts/quick_timer_resume_audit.py':['quick_timer_resume_audit','prompt_free_timer_snapshot'],
 'scripts/quick_bfcache_resume_audit.py':['quick_bfcache_resume_audit','matching_bfcache_snapshot_reload'],
 'scripts/quick_background_pause_audit.py':['quick_background_pause_audit','hidden_auto_pause'],
 'scripts/quick_hidden_snapshot_audit.py':['quick_hidden_snapshot_audit','hidden_snapshot_without_pagehide'],
...
for module in ('party-wave-one-voting-catalog.js','party-wave-one-bluff-catalog.js','party-wave-one-clue-catalog.js','party-wave-one-voting-modes.js','party-wave-one-bluff-modes.js','party-wave-one-clue-modes.js','quick-loader.js'):
    if f'node --check {module}' not in syntax_gate: violations.append(f'Production module missing from syntax gate: {module}')
for test in ('tests/party-wave-one-voting-catalog.test.js','tests/party-wave-one-bluff-catalog.test.js','tests/party-wave-one-clue-catalog.test.js','tests/quick-loader.test.js','tests/party-release-structure.test.js'):
    if test not in unit_gate: violations.append(f'Critical final Wave 1 test missing from npm test: {test}')
if 'node --check tests/e2e/wave-one-remaining.spec.js' not in syntax_gate: violations.append('Final Wave 1 E2E missing from syntax gate')
for audit in ('scripts/wave_one_quiz_audit.py','scripts/wave_one_imposter_audit.py','scripts/wave_one_writing_audit.py','scripts/wave_one_remaining_audit.py','scripts/advanced_integrity_audit.py','scripts/quick_session_replacement_audit.py','scripts/quick_timer_resume_audit.py','scripts/quick_bfcache_resume_audit.py','scripts/quick_background_pause_audit.py','scripts/quick_hidden_snapshot_audit.py','scripts/backup_contract_audit.py','scripts/core_content_audit.py','scripts/reference_content_audit.py','scripts/asset_provenance_audit.py','scripts/media_inventory_audit.py','scripts/public_release_placeholder_audit.py','scripts/hub_a11y_contract_audit.py','scripts/secondary_surface_a11y_contract_audit.py','scripts/operator_release_contract_audit.py','scripts/release_audit.py','scripts/performance_budget.py'):
...
print(json.dumps({
 'architecture_audit':'PASS','production_modules':len(production_js),'html_pages':len(html_pages),'runtime_dependencies':0,
 'module_line_limit':1000,'module_size_limit_bytes':100000,'pwa_cache':cache.group(1) if cache else None,'catalog_chain':catalog_chain,
 'built_in_games':55,'release_tiers':{'core':15,'extended':13,'labs':27},'quick_loader_version':11,'session_controls_version':5,
 'wave_one_complete':10,'wave_one_audits':['quiz','imposter','writing','remaining'],'wave_one_shared_engine_families':6,
 'wave_one_private_handoff_concealment':True,'wave_one_private_entry_concealment':True,'wave_one_derived_result_resume':True,
 'wave_one_adult_content':False,'hub_accessibility_layer_audited':True,'reference_content_audit_required':True,
 'backup_registry_version':2,'shared_session_controls':True,'exact_once_contract':True,'controlled_pwa_update':True
