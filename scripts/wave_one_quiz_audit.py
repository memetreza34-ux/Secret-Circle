...
required = [
    'GAME_LIBRARY_BACKLOG.json', 'APP_SPIELMODI_UND_THEMEN_ANLEITUNG.md',
    'party-wave-one-catalog.js', 'party-wave-one-modes.js', 'quick-loader.js',
    'party-release-structure.js', 'party.html', 'quick-play.html', 'sw.js',
    'tests/party-wave-one-catalog.test.js', 'tests/quick-loader.test.js',
    'tests/party-release-structure.test.js', 'tests/e2e/wave-one-quiz.spec.js', 'package.json'
]
...
runner = read('party-wave-one-modes.js')
...
unit = read('tests/party-wave-one-catalog.test.js')
e2e = read('tests/e2e/wave-one-quiz.spec.js')
...
for marker in (
    "ACTIVE_KEY = 'secret-circle-party-quick-active-v1'", "L.completionId('wave1', game.id, active.sessionId)",
    "if (gameId === 'party-quiz')", "if (gameId === 'fact-or-fake')",
...
if 'node tests/party-wave-one-catalog.test.js' not in scripts.get('test', ''):
    violations.append('Wave 1 catalog test missing from npm test')
...
if 'scripts/wave_one_quiz_audit.py' not in scripts.get('validate', ''):
    violations.append('Wave 1 audit missing from npm run validate')
...
for marker in (
    'Party Quiz resolves exactly once and keeps the result stable through reload/resume',
    'Fake oder Fakt is playable as a separate Wave 1 Labs mode',
    'Wave 1 games share Quick-family replacement protection across games'
):
    if marker not in e2e: violations.append(f'Wave 1 browser marker missing: {marker}')

if violations: raise SystemExit('\n'.join(sorted(set(violations))))
print(json.dumps({
    'wave_one_quiz_audit': 'PASS',
    'implemented_labs': ['party-quiz', 'fact-or-fake'],
    'shared_runner': True,
