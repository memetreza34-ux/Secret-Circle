...
for relative in required:
    if not (ROOT / relative).is_file():
        violations.append(f'Missing BF58 contract file: {relative}')
if violations:
    raise SystemExit('\n'.join(violations))
...
for marker in (
    ...
    if marker not in controls:
        violations.append(f'BF58 controller marker missing: {marker}')
...
for marker in (
    ...
    if marker not in unit:
        violations.append(f'BF58 unit marker missing: {marker}')
...
for marker in (
    ...
    if marker not in e2e:
        violations.append(f'BF58 browser marker missing: {marker}')

scripts = package.get('scripts', {})
if 'node tests/party-session-controls.test.js' not in scripts.get('test', ''):
    violations.append('BF58 unit missing from npm test.')
if 'node --check tests/e2e/quick-timer-resume.spec.js' not in scripts.get('check', ''):
    violations.append('BF58 browser spec missing from syntax preflight.')
if 'scripts/quick_bfcache_resume_audit.py' not in scripts.get('validate', ''):
    violations.append('BF58 audit missing from npm validate.')
if scripts.get('test:e2e') != 'playwright test':
    violations.append('BF58 browser contract missing from full Playwright gate.')

cache = re.search(r"const CACHE='secret-circle-v(\d+)'", sw)
if not cache or int(cache.group(1)) < 58:
    violations.append('BF58 requires cache v58 or newer.')
if cache and f"secret-circle-v{cache.group(1)}" not in sw_test:
    violations.append('BF58 SW test cache drift.')
...
print(json.dumps({
    ...
    'browser_lifecycle_contract': True,
