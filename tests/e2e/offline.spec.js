...
test('service worker caches the complete offline core', async ({ page, context }) => {
  ...
  const cacheState = await page.evaluate(async () => {
    ...
    const cache = await caches.open('secret-circle-v9');
    const expected = [
  ...
  expect(cacheState.names).toContain('secret-circle-v9');
  expect(cacheState.names.filter(name => name.startsWith('secret-circle-'))).toEqual(['secret-circle-v9']);
