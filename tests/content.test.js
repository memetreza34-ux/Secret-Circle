...
for (const [id, entries] of Object.entries(C.PACKS)) {
  assert.ok(C.LABELS[id]);
  assert.equal(entries.length, 10);
  assert.equal(new Set(entries.map((entry) => entry[0].toLocaleLowerCase('de-DE'))).size, entries.length);
  assert.ok(entries.every(([word, hint]) => word.length >= 2 && hint.length >= 2));
}
console.log(
  JSON.stringify(
    {
      ok: true,
      version: C.VERSION,
      packs: C.METADATA.packCount,
      entries: C.METADATA.entryCount,
      familyFriendly: true,
