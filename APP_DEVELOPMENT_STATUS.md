...
v45 Core → v46 Hub-A11y → v47 Secondary-A11y → v48 Word-Imposter → v49 Hub Resume Guard → v50 fail-closed Loader → v51 Complete Backup → v52 sichere Hub-Current-Runden → v53 Paranoia Resume/Privacy → v54 Pre-Timer Resume → v55 Advanced Integrity → **v56 Quick Session Replacement**.
...
| 6 | Architektur / ADR | PREPARED | v56-PWA-/QR56-Evidence |
...
| 10 | Fehlerbehandlung / Resilienz | PREPARED | Quota/Update/Resume/Restore/Replacement real |
| 11 | Tests / CI | BLOCKED | funktionierender Hosted Runner |
| 12 | Offline / PWA / Resume | PREPARED | v56 Install/Upgrade/Rollback + Spezialgates |
| 13 | Content / Alter / Privacy | IN PROGRESS | reale Gruppen + finaler Sign-off |
| 14 | Beta / reale Gruppen | PREPARED | G1–G5 + DWI/HR2/BK51/HR52/PR53/PT54/AD55/QR56 + PN1–PN3 |
...
- `quick-session-replacement-guard.js` Version 1 schützt die Active-Keys von Quick/Trending, Mega, Viral und Creator.
- `quick-loader.js` Version 7 lädt Ledger → Controls → Replacement Guard → Engine.
- Same-Game- und Cross-Game-Ersatz innerhalb derselben Familie benötigen eine Verwerfbestätigung.
- Cancel erhält den Alt-Snapshot unverändert.
- der alte Snapshot wird nicht vorzeitig gelöscht; erst ein erfolgreicher Engine-Write ersetzt ihn.
- ein Replacement-Write-Fehler wird fail-closed behandelt: `pagehide`-Retry wird blockiert, kontrollierter Reload, Alt-Snapshot bleibt erhalten.
- `tests/quick-session-replacement-guard.test.js`, `tests/e2e/quick-session-replacement.spec.js` und `tests/e2e/party-session-controls.spec.js` schützen Daten- und Load-Order-Verträge.
- `scripts/quick_session_replacement_audit.py` ist Teil von `npm run validate`.

Realer Nachweis: **QR56**.
...
- alle früheren Resume-/Privacy-/A11y-/Backup-/Timer-/Advanced-Verträge bleiben enthalten
...
- QR56-Audit + Guard-/Loader-/Browser-Syntaxchecks
...
- **#8:** reale Geräte, v56 Offline-PWA, Accessibility, DWI, HR2, BK51, HR52, PR53, PT54, AD55, QR56 und Partytests
...
5. DWI + HR2 + BK51 + HR52 + PR53 + PT54 + AD55 + QR56 real
